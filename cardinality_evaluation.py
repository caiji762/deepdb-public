...
#     # queries 来源于 pull operator，所以pull operator 要能够连接上miniob
#     # read all queries
#     with open(query_filename) as f:
#         queries = f.readlines()
...
#         # 得到的 cardinality estimate 要通过push operator 送给 miniob
...
#         csv_rows.append({'query_no': query_no, 'q-error': q_error})
#         # csv_rows.append({'query_no': query_no,
#         #                  'query': query_str,
#         #                  'q-error': q_error,
...
#     ibjs_vals = [1.23, 1.59, 150, 3198, 14309, 590, 1000]   # 第一个25分位数瞎写的, 最后一个最大值也是瞎写的
#     mcsn_vals = [1.23, 3.82, 78.4, 362, 927, 57.9, 1000]    # 同上
#     for i, percentile in enumerate([25, 50, 90, 95, 99]):
#         logger.info(f"Q-Error {percentile}%-Percentile: {np.percentile(q_errors, percentile)} (vs. "
#                     f"MCSN: {mcsn_vals[i]} and IBJS: {ibjs_vals[i]})")
#     logger.info(f"Q-Max wo inf {np.max(q_errors[np.isfinite(q_errors)])} (vs. "
...
def evaluate_cardinalities(ensemble_location, query, schema,
                           rdc_spn_selection, pairwise_rdc_path, use_generated_code=False,
                           max_variants=1, merge_indicator_exp=False, exploit_overlapping=False, min_sample_ratio=0):
    # load ensemble
    spn_ensemble = read_ensemble(ensemble_location, build_reverse_dict=True)
